...
import { FiPlus, FiFileText, FiSearch, FiEye, FiTrash2, FiEdit, FiDownload, FiUsers, FiRefreshCw } from 'react-icons/fi';
...
function FormsTab() {
  ...
  const [activeView, setActiveView] = useState('templates'); // 'templates', 'filled'
  ...
  const fetchFilledForms = async () => {
    try {
      // This would need a new API endpoint to get all filled forms
  ...
  const handleFormSelected = (accordForm, contact, policies) => {
    // This function is now handled by AccordFormSearchModal directly
    // The form will open in PandaDoc instead of the internal form filler
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-3">
          <button
            onClick={() => {
              // Check for completed PandaDoc documents
              const session = localStorage.getItem('pandadoc_session');
              if (session) {
                showInfo('📄 Checking for completed documents from PandaDoc...');
                // In a real implementation, this would check document status
                // and refresh the completed documents list
              }
            }}
            className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center"
          >
            <FiRefreshCw className="mr-2" />
            Refresh from PandaDoc
          </button>
          <button
            onClick={handleNewAccordForm}
            className="px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold rounded-xl shadow-lg hover:from-green-600 hover:to-emerald-700 transition-all duration-200 transform hover:scale-105 flex items-center"
      ...
      {filledForms.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filledForms.map(form => (
            <div key={form.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-lg transition-shadow">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
