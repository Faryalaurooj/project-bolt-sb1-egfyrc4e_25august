...
import { FiX, FiLock, FiStar, FiHelpCircle, FiArrowLeft, FiFileText, FiMail, FiDownload } from 'react-icons/fi';
import ReportStructureSelection from './ReportStructureSelection';
import ReportDateSelection from './ReportDateSelection';
import { useToast } from '../../hooks/useToast';

function OwnerOverviewReportModal({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    asOfDateRange: 'Date Range',
    displayTotals: 'Territory',
    addOns: 'Include Add Ons'
  });
  ...
  const handleSaveReport = () => {
    showInfo('📊 Report saved for future use!');
  };

  const handleRunReport = (format) => {
    showSuccess(`📈 Running Owner Overview report as ${format}...`);
  };
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="bg-pink-50 border border-pink-200 rounded p-4">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <span className="font-semibold text-purple-600">Owner Overview:</span>
                    <span className="text-gray-700 ml-2">
                      This report lists the details of receipts collected within the given date range, broken down by 
                      category (new, renewal, fees, etc). The result totals displayed are determined by the business structure level chosen in the 
                      'Display Totals' drop down.
                    </span>
                  </div>
                </div>
              </div>
...
export default OwnerOverviewReportModal;