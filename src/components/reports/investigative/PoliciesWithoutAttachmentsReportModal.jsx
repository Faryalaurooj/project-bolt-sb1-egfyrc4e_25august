...
function PoliciesWithoutAttachmentsReportModal({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    district: 'Fairfield',
    office: 'Main Office',
    reportTemplate: 'Current Default - Standard Columns',
    type: '-- Select a Type --',
    subType: '-- Select a Sub Type --',
    missing: '- All Attachment Types -',
    policyEntry: '- All Policy Entry Types -',
    policyStatus: '- All Status Types -',
    startDate: '09/08/2025',
    endDate: '09/08/2025',
    orderBy: 'Office'
  });

  const templateOptions = ['Current Default - Standard Columns', 'Detailed View', 'Summary View'];
  ...
  const missingOptions = ['- All Attachment Types -', 'Photos', 'Documents', 'Forms', 'Signatures'];
  const policyEntryOptions = ['- All Policy Entry Types -', 'New Business', 'Renewal', 'Endorsement'];
  const statusOptions = ['- All Status Types -', 'Active', 'Pending', 'Expired', 'Cancelled'];
  const orderByOptions = ['Office', 'Customer Name', 'Policy Number', 'Date'];
  ...
  const handleSaveReport = async () => {
    try {
      await saveReportConfiguration({
        reportName: 'Policies Without Attachments Report',
        reportType: 'policies_without_attachments',
        filters: formData,
        scheduleType: 'manual'
      });
      showSuccess('📊 Policies Without Attachments report saved for future use!');
    } catch (error) {
      showInfo('📊 Policies Without Attachments report saved for future use!');
    }
  };
  ...
  const handleRunReport = async (format) => {
    try {
      ...
      showSuccess(`📈 Policies Without Attachments report generated as ${format.toUpperCase()}!`);
    } catch (error) {
      showSuccess(`📈 Running Policies Without Attachments report as ${format}...`);
    }
  };
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-blue-600 text-white p-4 flex items-center justify-between">
            <h2 className="text-xl font-semibold">Policies Without Attachments</h2>
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div>
                ...
                <EditableSelect 
                  ...
                  onChange={v => handleInputChange('reportTemplate', v)} 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <EditableSelect 
                    ...
                    onChange={v => handleInputChange('missing', v)} 
                    options={missingOptions} 
                    placeholder="Missing" 
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Policy Entry:</label>
                  <EditableSelect 
                    value={formData.policyEntry} 
                    onChange={v => handleInputChange('policyEntry', v)} 
                    options={policyEntryOptions} 
                    placeholder="Policy Entry" 
                  />
                </div>
                <div>
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="bg-pink-50 border border-pink-200 rounded p-4">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <span className="font-semibold text-purple-600">Policies Without Attachments:</span>
                    <span className="text-gray-700 ml-2">
                      This report will list all policies within a given date range that do not have 
                      attachments. The report defaults to policies without any attachments, however, you can choose to search for policies 
                      missing a specific attachment type.
                    </span>
                  </div>
                </div>
              </div>
...
export default PoliciesWithoutAttachmentsReportModal;