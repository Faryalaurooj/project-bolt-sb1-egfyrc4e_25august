...
function CrossSellingReportModal({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    orderBy: 'Agent Of Record',
    customersWith: '- Select Line Of Business -',
    thatDoNotHave: '- Select Line Of Business -'
  });
  ...
  const orderByOptions = ['Agent Of Record', 'Customer Name', 'Office', 'Date'];
  ...
  const handleSaveReport = async () => {
    try {
      await saveReportConfiguration({
        reportName: 'Cross-Selling Report',
        reportType: 'cross_selling',
        filters: formData,
        scheduleType: 'manual'
      });
      showSuccess('📊 Cross-Selling report saved for future use!');
    } catch (error) {
      showInfo('📊 Cross-Selling report saved for future use!');
    }
  };
  ...
  const handleRunReport = async (format) => {
    try {
      ...
      showSuccess(`📈 Cross-Selling report generated as ${format.toUpperCase()}!`);
    } catch (error) {
      showSuccess(`📈 Running Cross-Selling report as ${format}...`);
    }
  };
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-blue-600 text-white p-4 flex items-center justify-between">
            <h2 className="text-xl font-semibold">Cross-Selling Report</h2>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Customers with:</label>
                  <EditableSelect 
                    value={formData.customersWith} 
                    onChange={v => handleInputChange('customersWith', v)} 
                    options={lineOfBusinessOptions} 
                    placeholder="Customers with" 
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">That do not have:</label>
                  <EditableSelect 
                    value={formData.thatDoNotHave} 
                    onChange={v => handleInputChange('thatDoNotHave', v)} 
                    options={lineOfBusinessOptions} 
                    placeholder="That do not have" 
                  />
                </div>
              </div>
            </div>
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="bg-pink-50 border border-pink-200 rounded p-4">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <span className="font-semibold text-purple-600">Cross-Selling Report:</span>
                    <span className="text-gray-700 ml-2">
                      This report will return Customers with one type of policy, who do not have an Active policy of 
                      another type. For example: All customers with Home insurance who do not currently have Flood insurance.
                    </span>
                  </div>
                </div>
              </div>
...
export default CrossSellingReportModal;