...
function BrokerListReportModal({ isOpen, onClose }) {
  ...
  const brokerStatusOptions = ['Active', 'Inactive', 'All Status'];
  const licenseTypeOptions = ['- All License Types -', 'Property & Casualty', 'Life & Health', 'Variable Annuity'];
  const orderByOptions = ['Broker Name', 'License Number', 'Office', 'License Type'];
  ...
  const handleSaveReport = async () => {
    try {
      await saveReportConfiguration({
        reportName: 'Broker List Report',
        reportType: 'broker_list',
        filters: formData,
        scheduleType: 'manual'
      });
      showSuccess('📊 Broker List report saved for future use!');
    } catch (error) {
      showInfo('📊 Broker List report saved for future use!');
    }
  };
  ...
  const handleRunReport = async (format) => {
    try {
      ...
      showSuccess(`📈 Broker List report generated as ${format.toUpperCase()}!`);
    } catch (error) {
      showSuccess(`📈 Running Broker List report as ${format}...`);
    }
  };
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-blue-600 text-white p-4 flex items-center justify-between">
            <h2 className="text-xl font-semibold">Broker List</h2>
          ...
          <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <EditableSelect 
                    ...
                    onChange={v => handleInputChange('brokerStatus', v)} 
                    options={brokerStatusOptions} 
                    placeholder="Broker Status" 
                  />
                </div>
                <div>
                  ...
                  <EditableSelect 
                    ...
                    onChange={v => handleInputChange('licenseType', v)} 
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="flex items-center space-x-4">
                <label className="text-sm font-medium text-gray-700">Include Contact Info:</label>
                <button
                  onClick={() => handleToggle('includeContactInfo')}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    formData.includeContactInfo ? 'bg-green-500' : 'bg-red-500'
                  }`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      formData.includeContactInfo ? 'translate-x-6' : 'translate-x-1'
                    }`}
                  />
                </button>
                <span className="text-sm text-gray-600">
                  {formData.includeContactInfo ? 'Yes' : 'No'}
                </span>
              </div>
            </div>
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="bg-pink-50 border border-pink-200 rounded p-4">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <span className="font-semibold text-purple-600">Broker List:</span>
                    <span className="text-gray-700 ml-2">
                      This report lists all brokers and their licensing information, including contact details and license status.
                    </span>
                  </div>
                </div>
              </div>
...
export default BrokerListReportModal;