...
function CSRReportModal({ isOpen, onClose }) {
  ...
  const csrOptions = ['- All CSR -', 'Sarah Wilson', 'Tom Brown', 'Lisa Davis', 'Mike Johnson'];
  const activityTypeOptions = ['All Activities', 'Policy Changes', 'Customer Updates', 'Claims Processing'];
  const orderByOptions = ['CSR Name', 'Activity Count', 'Office', 'Date'];
  ...
  const handleSaveReport = async () => {
    try {
      await saveReportConfiguration({
        reportName: 'CSR Report',
        reportType: 'csr',
        filters: formData,
        scheduleType: 'manual'
      });
      showSuccess('📊 CSR report saved for future use!');
    } catch (error) {
      showInfo('📊 CSR report saved for future use!');
    }
  };
  ...
  const handleRunReport = async (format) => {
    try {
      ...
      showSuccess(`📈 CSR report generated as ${format.toUpperCase()}!`);
    } catch (error) {
      showSuccess(`📈 Running CSR report as ${format}...`);
    }
  };
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <EditableSelect 
                    value={formData.csr} 
                    onChange={v => handleInputChange('csr', v)} 
                    options={csrOptions} 
                    placeholder="CSR" 
                  />
                </div>
                ...
                <div>
                  ...
                  <EditableSelect 
                    ...
                    onChange={v => handleInputChange('activityType', v)} 
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="flex items-center space-x-4">
                <label className="text-sm font-medium text-gray-700">Include Inactive CSR:</label>
                <button
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="bg-pink-50 border border-pink-200 rounded p-4">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <span className="font-semibold text-purple-600">CSR Report:</span>
                    <span className="text-gray-700 ml-2">
                      This report displays Customer Service Representative activity and performance metrics within the specified date range.
                    </span>
                  </div>
                </div>
              </div>
