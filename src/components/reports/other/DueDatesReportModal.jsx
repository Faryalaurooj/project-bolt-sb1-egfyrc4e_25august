...
function DueDatesReportModal({ isOpen, onClose }) {
  ...
  const dueTypeOptions = ['All Due Types', 'Policy Renewals', 'Premium Payments', 'Document Submissions'];
  const companyOptions = ['- All Companies -', 'State Farm', 'Allstate', 'Progressive', 'Travelers'];
  const orderByOptions = ['Due Date', 'Customer Name', 'Policy Number', 'Office'];
  ...
  const handleSaveReport = async () => {
    try {
      ...
      showSuccess('📊 Due Dates report saved for future use!');
    } catch (error) {
      showInfo('📊 Due Dates report saved for future use!');
    }
  };
  ...
  const handleRunReport = async (format) => {
    try {
      ...
      showSuccess(`📈 Due Dates report generated as ${format.toUpperCase()}!`);
    } catch (error) {
      showSuccess(`📈 Running Due Dates report as ${format}...`);
    }
  };
          ...
          <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <EditableSelect 
                    ...
                    onChange={v => handleInputChange('dueType', v)} 
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="flex items-center space-x-4">
                <label className="text-sm font-medium text-gray-700">Include Overdue:</label>
                <button
                  onClick={() => handleToggle('includeOverdue')}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    formData.includeOverdue ? 'bg-green-500' : 'bg-red-500'
                  }`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      formData.includeOverdue ? 'translate-x-6' : 'translate-x-1'
                    }`}
                  />
                </button>
                <span className="text-sm text-gray-600">
                  {formData.includeOverdue ? 'Yes' : 'No'}
                </span>
              </div>
            </div>
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="bg-pink-50 border border-pink-200 rounded p-4">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <span className="font-semibold text-purple-600">Due Dates:</span>
                    <span className="text-gray-700 ml-2">
                      This report lists all items with due dates within the specified date range, including policy renewals and payment due dates.
                    </span>
                  </div>
                </div>
              </div>
...
export default DueDatesReportModal;