...
function HiddenInactiveReportModal({ isOpen, onClose }) {
  ...
  const recordTypeOptions = ['All Records', 'Customers', 'Policies', 'Documents'];
  const statusOptions = ['Hidden & Inactive', 'Hidden Only', 'Inactive Only'];
  const orderByOptions = ['Customer Name', 'Date Hidden', 'Date Inactivated', 'Office'];
  ...
  const handleSaveReport = async () => {
    try {
      await saveReportConfiguration({
        reportName: 'Hidden & Inactive Report',
        reportType: 'hidden_inactive',
        filters: formData,
        scheduleType: 'manual'
      });
      showSuccess('📊 Hidden & Inactive report saved for future use!');
    } catch (error) {
      showInfo('📊 Hidden & Inactive report saved for future use!');
    }
  };
  ...
  const handleRunReport = async (format) => {
    try {
      ...
      showSuccess(`📈 Hidden & Inactive report generated as ${format.toUpperCase()}!`);
    } catch (error) {
      showSuccess(`📈 Running Hidden & Inactive report as ${format}...`);
    }
  };
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-blue-600 text-white p-4 flex items-center justify-between">
            <h2 className="text-xl font-semibold">Hidden & Inactive</h2>
          ...
          <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <EditableSelect 
                    ...
                    onChange={v => handleInputChange('recordType', v)} 
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="flex items-center space-x-4">
                <label className="text-sm font-medium text-gray-700">Include Reasons:</label>
                <button
                  onClick={() => handleToggle('includeReasons')}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    formData.includeReasons ? 'bg-green-500' : 'bg-red-500'
                  }`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      formData.includeReasons ? 'translate-x-6' : 'translate-x-1'
                    }`}
                  />
                </button>
                <span className="text-sm text-gray-600">
                  {formData.includeReasons ? 'Yes' : 'No'}
                </span>
              </div>
            </div>
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="bg-pink-50 border border-pink-200 rounded p-4">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <span className="font-semibold text-purple-600">Hidden & Inactive:</span>
                    <span className="text-gray-700 ml-2">
                      This report lists all records that have been marked as hidden or inactive in the system.
                    </span>
                  </div>
                </div>
              </div>
...
export default HiddenInactiveReportModal;