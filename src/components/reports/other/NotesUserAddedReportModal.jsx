...
function NotesUserAddedReportModal({ isOpen, onClose }) {
  ...
  const addedByOptions = ['- All Users -', 'John Smith', 'Jane Doe', 'Mike Johnson', 'Sarah Wilson'];
  const noteTypeOptions = ['All User Notes', 'Customer Notes', 'Policy Notes', 'Action Items'];
  const orderByOptions = ['Date Added', 'Customer Name', 'Added By', 'Note Type'];
  ...
  const handleSaveReport = async () => {
    try {
      await saveReportConfiguration({
        reportName: 'Notes - User Added Report',
        reportType: 'notes_user_added',
        filters: formData,
        scheduleType: 'manual'
      });
      showSuccess('📊 Notes - User Added report saved for future use!');
    } catch (error) {
      showInfo('📊 Notes - User Added report saved for future use!');
    }
  };
  ...
  const handleRunReport = async (format) => {
    try {
      ...
      showSuccess(`📈 Notes - User Added report generated as ${format.toUpperCase()}!`);
    } catch (error) {
      showSuccess(`📈 Running Notes - User Added report as ${format}...`);
    }
  };
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-blue-600 text-white p-4 flex items-center justify-between">
            <h2 className="text-xl font-semibold">Notes - User Added</h2>
          ...
          <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <EditableSelect 
                    ...
                    onChange={v => handleInputChange('addedBy', v)} 
                ...
                <div>
                  ...
                  <EditableSelect 
                    ...
                    onChange={v => handleInputChange('noteType', v)} 
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="flex items-center space-x-4">
                <label className="text-sm font-medium text-gray-700">Include Note Content:</label>
                <button
                  onClick={() => handleToggle('includeContent')}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    formData.includeContent ? 'bg-green-500' : 'bg-red-500'
                  }`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      formData.includeContent ? 'translate-x-6' : 'translate-x-1'
                    }`}
                  />
                </button>
                <span className="text-sm text-gray-600">
                  {formData.includeContent ? 'Yes' : 'No'}
                </span>
              </div>
            </div>
            ...
            <div className="border border-gray-300 rounded-lg p-4">
              ...
              <div className="bg-pink-50 border border-pink-200 rounded p-4">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <span className="font-semibold text-purple-600">Notes - User Added:</span>
                    <span className="text-gray-700 ml-2">
                      This report displays all notes that were manually added by users within the specified date range.
                    </span>
                  </div>
                </div>
              </div>
...
export default NotesUserAddedReportModal;