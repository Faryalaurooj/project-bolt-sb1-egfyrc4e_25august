...
const DashboardLayout = () => {
  ...
  const openTextCampaignModal = () => {
    console.log('🔍 Opening text campaign modal...');
      ...
      <main className="flex-1 p-4 md:p-8 bg-gradient-to-tr from-[#F9FAF9] via-[#F5FFF9] to-white dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 relative overflow-hidden">
        <Draggable
          ...
          onStop={(_, data) => {
            ...
            localStorage.setItem('calendarPosition', JSON.stringify(newPos));
          }}
        >
          <div className="w-fit mb-6 cursor-move absolute z-40">
            <Calendar onChange={setCalendarValue} value={calendarValue} className="rounded-md shadow border border-green-300" />
          </div>
        </Draggable>
